  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      setConversations((prevConvs) => {
        const updateConvo = prevConvs.map(conversation => {
          if (conversation._id === selectedConversation._id) {
            return { ...conversation, lastMessage: { text: messageText, sender: data.sender } }
          }
          return conversation
        })
        return updateConvo
  ...
  return (
    <Flex gap={2} alignItems={"center"}>
      ...
      <form onSubmit={handleSendMessage} style={{ flex: 95 }}>
        <InputGroup>
          <Input w={"full"} placeholder='Typr a message' onChange={(e) => setMessageText(e.target.value)} value={messageText} />
